...
6. **문체 검사 결과** — REVIEW_FINALIZE 7번에서 `output/01_대본.txt`에 돌린 최종 결과를 한 줄로 적는다
   - 통과: `문체 검사 ✅ 통과 (확인 N건)` — `[확인]` 항목이 있으면 건수와 무엇인지 한 줄 덧붙인다
   - 미통과 상태로 여기까지 온 경우: **통과했다고 쓰지 않는다.** 무엇이 몇 건 남았는지 그대로 적는다
   - 린터를 돌리지 않았으면 그 사실을 적는다. **돌린 척하지 않는다**
7. **완성본 폴더 열기**: OS에 맞게 `{P}/output` 폴더를 파일 탐색기로 열어준다 (macOS: `open "{P}/output"` / Windows: `explorer "{P}\output"`). 실패해도 중단하지 않고 경로 안내로 대체
8. 마지막 줄 고정 안내 (생략 금지):
